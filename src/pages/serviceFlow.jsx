...
export function ServiceCategoriesPage() {
  ...
  const plans = useMemo(() => {
    ...
    return data.plans.filter((item) => isActive(item) && relationId(item.childServiceId) === selectedChild._id);
  }, [data, selectedChild]);
...
export function ServicePlansPage() {
  ...
  const plans = useMemo(() => { if (!data || !child) return []; const needle = query.toLowerCase(); return data.plans.filter((item) => isActive(item) && relationId(item.childServiceId) === child._id).filter((item) => !needle || [item.title, item.slug, item.description].some((value) => String(value || "").toLowerCase().includes(needle))); }, [child, data, query]);
